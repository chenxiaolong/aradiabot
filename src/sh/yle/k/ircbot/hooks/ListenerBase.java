...
public abstract class ListenerBase<T extends IRCBot> implements Listener<T> {
	...
	/**
	 * Determines if the Listener is enabled.
	 **/
	...
	/**
	 * The generic Event trigger which calls other
	 * event methods after determining the type of event that
	 * was initially triggered. This will also trigger the
	 * generic version of the event.
	 * 
	 * If the listener is disabled, it will automatically
	 * return out of the method and not process the event.
	 **/
	@SuppressWarnings("unchecked")
	public void onEvent(Event<T> event) throws Exception {
		if (!enabled) return; //Cancel Event if disabled
	...
	/**
	 * All of the following methods are predefined so they can be overwritten
	 * and called later from the onEvent() method. Since these are not abstract,
	 * they are optional.
	 **/
